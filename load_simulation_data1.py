...
for random_seed in seed_pool:
    ...
    beta=np.array([5,5,3,0,0,0,0,0])

    np.random.seed(random_seed)
    cordataY=np.dot(cordataX, beta)+np.random.normal(0,7,num_samples)
    ...
    cordataY=(cordataY-np.mean(cordataY))/std_Y
...
np.savetxt('OSCAR_MSE_log1.txt', OSCAR_MSE_log)

np.savetxt('Lasso_MSE_log1.txt', Lasso_MSE_log)

np.savetxt('Elastic_net_MSE_log1.txt', Elastic_net_MSE_log)

np.savetxt('OSCAR_df_log1.txt', OSCAR_df_log)

np.savetxt('Lasso_df_log1.txt', Lasso_df_log)

np.savetxt('Elastic_net_df_log1.txt', Elastic_net_df_log)

np.savetxt('clf_MSE_log1.txt', clf_MSE_log)

np.savetxt('clf_df_log1.txt', clf_df_log)


print('OSCAR MSE:', np.mean(OSCAR_MSE_log), '. STD:', np.std(OSCAR_MSE_log))
print('Ridge MSE:', np.mean(clf_MSE_log), '. STD:', np.std(clf_MSE_log))
print('Lasso MSE:', np.mean(Lasso_MSE_log), '. STD:', np.std(Lasso_MSE_log))
print('Elastic Net MSE:', np.mean(Elastic_net_MSE_log), '. STD:', np.std(Elastic_net_MSE_log))
