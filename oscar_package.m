...
function [op_coef, df] = oscar_package()

format long

X_train = readmatrix("OSCAR_X_train.csv");
X_test = readmatrix("OSCAR_X_test.csv");
y_train = readmatrix("OSCAR_y_train.csv");
y_test = readmatrix("OSCAR_y_test.csv");

cvalues = [0; .01; .05; .1;.25;.5;.55;.6;.7;.75;.8;.85;.9;.95;1];
propvalues = [.0001; .001; .002; .00225; .0025; .00275; .0028; .0029; .003; .004; .005; .0075; .01;.025; .05; .1; .15;.2;.25;.3;.35;.4;.45;.5;.55;.6];
...
for ccount = 1:length(cvalues)
    ...
    for propcount = 1:length(propvalues)
        ...
        Test_Error_Matrix(propcount,ccount) = sum((X_test*coefs-y_test).^2);
        SSMatrix(:,propcount,ccount) = ssquares;
        dfMatrix(:,propcount,ccount) = df;
    end;
end;

[op_row, op_column]=find(Test_Error_Matrix==min(min(Test_Error_Matrix)));
op_row = op_row(1);
op_column = op_column(1);
df = dfMatrix(:,op_row, op_column);

op_coef=CoefMatrix(:,op_row, op_column);
